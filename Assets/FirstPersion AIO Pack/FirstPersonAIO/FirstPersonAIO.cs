...
/// 05/14/20 || msg: "Fixed some slope detection bugs" || Author: Aedan Graves || verison 20.5.9a >> 20.5.9b
...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        if(IsGrounded && didJump && jumpPowerInternal > 0){
            ...
            if(advanced.maxSlopeAngle>0 && IsGrounded && SlopeCheck()<=0.25f){yVelocity *= SlopeCheck();}
    ...
    private void FixedUpdate()
    {
        ...
        if(advanced.maxSlopeAngle>0){
            dMove = ((transform.forward * inputXY.y * speed + transform.right * inputXY.x * walkSpeedInternal) *(fps_Rigidbody.velocity.y > -0.5f ? SlopeCheck(): IsGrounded ? 0.85f : 1))+ Vector3.up*fps_Rigidbody.velocity.y;
        }
        else{
        ...
        RaycastHit WT;
        if(advanced.maxStepHeight > 0 && Physics.Raycast(transform.position - new Vector3(0,((capsule.height/2)*transform.localScale.y)-0.01f,0),dMove,out WT,capsule.radius+0.15f,Physics.AllLayers,QueryTriggerInteraction.Ignore) && Vector3.Angle(WT.normal, Vector3.up)>88){
            RaycastHit ST;
            if(!Physics.Raycast(transform.position - new Vector3(0,((capsule.height/2)*transform.localScale.y)-(advanced.maxStepHeight),0),dMove,out ST,capsule.radius+0.25f,Physics.AllLayers,QueryTriggerInteraction.Ignore)){
    ...
    float SlopeCheck(){
            advanced.lastKnownSlopeAngle =  (Vector3.Angle(advanced.curntGroundNormal, Vector3.up));
            return new AnimationCurve(new Keyframe(-90.0f, 1.0f), new Keyframe(0.0f, 1.0f), new Keyframe(advanced.maxSlopeAngle, 0.0f),new Keyframe(advanced.maxWallShear, 0.0f),new Keyframe(advanced.maxWallShear+0.1f, 1.0f),new Keyframe(90, 1.0f)){preWrapMode = WrapMode.Clamp, postWrapMode = WrapMode.ClampForever}.Evaluate(advanced.lastKnownSlopeAngle);
    }
    ...
    private void OnCollisionStay(Collision CollisionData) {
        if(IsGrounded || fps_Rigidbody.velocity.y < 0.1){
            ...
            for(int i = 0; i<CollisionData.contactCount; i++){
                if(CollisionData.GetContact(i).point.y < (transform.position.y - (capsule.radius+0.195f))){
                    if(advanced.maxSlopeAngle>0){
    ...
    private void OnCollisionExit(Collision CollisionData) {
        if(CollisionData.contactCount == 0 && IsGrounded){IsGrounded = false;}
    }
}
    ...
    public class FPAIO_Editor : Editor{
        ...
        void OnEnable(){
            ...
            DownloadImage("http://www.aedangraves.info/portfolio-site/MiscSite_content/Images/adTex1.png");
        }   
        ...
        #region Movement Setup
            ...
            if(showAdvanced){
                ...
                EditorGUILayout.Space();
                t.advanced.maxSlopeAngle = EditorGUILayout.Slider(new GUIContent("Max Slope Angle","Determines the maximum angle the player can walk up. If left 0, the slope detection/limiting system will not be used."),t.advanced.maxSlopeAngle,0,70);
                if(t.advanced.maxSlopeAngle>0){t.advanced.maxWallShear = EditorGUILayout.Slider(new GUIContent("Max Wall Shear","The Maximum amount of shear a wall can have before its registered as a slope."),t.advanced.maxWallShear,85,89);}
                EditorGUILayout.Space();
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            if(GUILayout.Button(adTex1, new GUIStyle(){stretchWidth = true,fixedHeight = 64, alignment = TextAnchor.MiddleCenter})){Application.OpenURL("https://assetstore.unity.com/packages/templates/systems/infinigun-universal-fps-firearm-153247");}
            EditorGUILayout.Space();
